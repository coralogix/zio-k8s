...
import zio.{ Clock, Layer, RIO, Ref, Scope, System, Task, ZIO, ZLayer }
...
package object config extends Descriptors {
  ...
  ): Task[ServiceAccountAuthData] =
    tokenSource match {
      case KeySource.FromFile(path) =>
        for {
          initialToken <- loadKeyString(tokenSource)
      ...
      case _                        =>
        loadKeyString(tokenSource).map { token =>
          ServiceAccountAuthData(
            authenticator = (request: RequestT[Empty, Either[String, String], Any]) =>
