...
/** Extension methods for Kubernetes resources
  ...
  * the resource's [[K8sObject]] implementation.
  *
  * @tparam T Resource type to be extended
  */
trait K8sObjectOps[T] {
  ...
  /** Creates a new object with its metadata modified by the given function f
    * @param f Function to modify the object's metadata with
    * @return Object with modified metadata
    */
  ...
  /** Attach another resource as the owner of this one
    ...
    * @param ownerType Owner's resource type metadata
    * @return Object with the attached owner described in its metadata
    */
  def attachOwner(
    ownerName: String,
    ownerUid: String,
  ...
  /** Tries to attach another resource as the owner of this one. Can fail with [[com.coralogix.zio.k8s.client.UndefinedField]]
    * if the owner does not have all the required metadata.
    *
    * @param owner Owner resource
    * @tparam OwnerT Type of the owner
    * @return Object with the attached owner described in its metadata
    */
  ...
  /** Verifies ownership between the resources
    * @param owner Possible owner of this resource
    * @tparam OwnerT Type of the owner resource
    * @return True if owner owns this resource
    */
