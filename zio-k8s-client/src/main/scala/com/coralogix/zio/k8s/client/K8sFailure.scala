...
/** Failed HTTP response
  *
  * Indicates that the response from the Kubernetes API has a non-successful status code and it's
  * body did not contain a [[com.coralogix.zio.k8s.model.pkg.apis.meta.v1.Status]] value.
  *
  * In case the error is a standard Kubernetes error message, the error type will be
  * [[DecodedFailure]].
  *
  * Note that some specific failure types are encoded by their own failure type. See
  * [[Unauthorized]], [[Gone]] and [[NotFound]].
  *
  * @param message
  *   Response message
  * @param code
...
/** Failed Kubernetes API request
  ...
  *
  * @param status
  *   The Kubernetes [[com.coralogix.zio.k8s.model.pkg.apis.meta.v1.Status]] value returned in the
  *   failure response
  * @param code
  *   Response status code
  */
...
/** Error indicating that Kubernetes API responded with success, but the response body could not be
  * deserialized to the expected data type.
  * @param error
